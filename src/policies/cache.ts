  ...
  return (fn: ActFn<T>, ctx: PolicyContext): ActFn<T> =>
    async (signal: AbortSignal) => {
      ...
      if (isSyncStore(ctx.store)) {
        ...
        const inflight = ctx.store.get<Promise<T>>(inflightKey)
        if (inflight) return inflight

        // 3. Originator: launch fn, cache on success (fail-open).
        const promise = Promise.resolve(fn(signal)).then(
          (value) => {
            try {
        ...
        try {
          ctx.store.set<Promise<T>>(inflightKey, promise)
          ...
          // try/catch via act().
          promise.finally(() => {
            try { ctx.store.delete(inflightKey) } catch { /* ignore */ }
          }).catch(() => { /* cleanup chain — swallow */ })
        } catch {
