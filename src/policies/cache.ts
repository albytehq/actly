...
// Wrap cached values in an object so `T = undefined` is still distinguishable
// from a cache miss (where `get()` returns `undefined`).
...
// In-flight single-flight entry carries a generation token (same
// pattern as dedupePolicy) so stale cleanup from an old originator doesn't
// clobber a newer entry. Also carries meta reference so joiners can
// mirror originator's attempt count and source.
  ...
  return (fn: ActFn<T>, ctx: PolicyContext): ActFn<T> =>
    async (signal: AbortSignal) => {
      ...
      if (isSyncStore(ctx.store)) {
        ...
        try {
          store.set<InflightEntry<T>>(inflightKey, { promise: rawPromise, generation, meta: ctx.meta })
        } catch {
 ...
 */
function raceWithOwnSignal<T>(promise: Promise<T>, signal: AbortSignal): Promise<T> {
  if (signal.aborted) {
