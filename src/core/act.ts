import type { ActFn, ActOptions, ActResult, PolicyApplier, RunMeta } from '../types/index.js'
...
export async function act<T>(
  ...
  if (options.totalTimeout && options.totalTimeout.ms > 0) {
    policies.push(totalTimeoutPolicy<T>(options.totalTimeout)) // 0. hardest outer wall
  }

  if (options.cache && options.cache.ttl > 0) {
    policies.push(cachePolicy<T>(options.cache))               // 1. skip all on hit
  }

  if (dedupe?.enabled) {
    policies.push(dedupePolicy<T>())                           // 2. collapse concurrent callers
  }

  if (options.retry && options.retry.attempts > 1) {
    policies.push(retryPolicy<T>(options.retry))               // 3. own the attempt loop
  }

  if (options.timeout && options.timeout.ms > 0) {
    policies.push(timeoutPolicy<T>(options.timeout))           // 4. innermost — per-attempt clock
  }
