...
import { InMemoryStore, createDefaultStore } from '../stores/memory.js'
import { isSyncStore }            from '../stores/base.js'
import { linkSignal, raceAbort }  from '../utils/abort.js'
import { acquireController, releaseController } from '../utils/abortPool.js'
 ...
 */
function classifyFailure(error: unknown): 'abort' | 'timeout' | 'total-timeout' | 'retry-exhausted' | 'fn-error' | 'validation' {
  ...
  try {
    ...
    if (options.fallback) {
      ...
      try {
        ...
        return { ok: true, value: fallbackValue as T, source: 'fresh', attempts: meta.attempts, traceId: observability?.traceId, durationMs }
      } catch {
...
function wrapHedge<T>(fn: ActFn<T>, delayMs: number): ActFn<T> {
