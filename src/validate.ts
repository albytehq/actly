...
import { OBSERVABILITY_HOOKS, type ObservabilityHooks } from './observability.js'

const HOOK_NAMES = new Set<string>(OBSERVABILITY_HOOKS)
...
/**
 * Validate user-facing option shapes. Throws RangeError / TypeError
 * synchronously (programmer errors, not runtime failures). Every numeric
 * field is bounded by LIMITS.
 */
...
export function assertRetryOptions(opts: RetryOptions): void {
  ...
  if (opts.attempts > LIMITS.MAX_RETRY_ATTEMPTS) {
    throw new RangeError(
      `Actly: retry.attempts ${opts.attempts} exceeds limit ${LIMITS.MAX_RETRY_ATTEMPTS}. Use an outer supervisor for more.`,
    )
  }
  ...
  if (opts.backoff !== undefined && !BACKOFF_MODES.has(opts.backoff)) {
    throw new RangeError(
      `Actly: retry.backoff must be one of ${[...BACKOFF_MODES].map((m) => JSON.stringify(m)).join(' | ')}, got ${JSON.stringify(opts.backoff)}`,
    )
  }
  if (opts.jitter !== undefined && !JITTER_MODES.has(opts.jitter)) {
    throw new RangeError(
      `Actly: retry.jitter must be one of ${[...JITTER_MODES].map((m) => JSON.stringify(m)).join(' | ')}, got ${JSON.stringify(opts.jitter)}`,
    )
  }
  ...
  if (opts.acceptResult !== undefined && typeof opts.acceptResult !== 'function') {
    throw new TypeError(`Actly: retry.acceptResult must be a function, got ${typeof opts.acceptResult}`)
  }
...
export function assertDedupeOptions(opts: DedupeOptions): void {
  if (typeof opts !== 'object' || opts === null) {
    throw new TypeError(`Actly: dedupe must be an object, got ${opts === null ? 'null' : typeof opts}`)
  }
  if (opts.enabled !== undefined && typeof opts.enabled !== 'boolean') {
    throw new TypeError(`Actly: dedupe.enabled must be a boolean, got ${typeof opts.enabled}`)
  }
  if (opts.inflightTtl !== undefined) {
    if (opts.inflightTtl === Number.POSITIVE_INFINITY) return
    if (opts.inflightTtl === 0) {
      throw new RangeError(
        'Actly: dedupe.inflightTtl must be > 0 (or Infinity). Use a small positive number for near-immediate expiry.',
      )
    }
...
export function assertObservabilityHooks(hooks: ObservabilityHooks): void {
  if (typeof hooks !== 'object' || hooks === null) {
    throw new TypeError(`Actly: observability must be an object, got ${hooks === null ? 'null' : typeof hooks}`)
  }
  for (const name of Object.keys(hooks)) {
    if (!HOOK_NAMES.has(name)) {
      throw new TypeError(
        `Actly: unknown observability hook "${name}". Valid hooks: ${OBSERVABILITY_HOOKS.join(', ')}.`,
      )
    }
    const value = (hooks as Record<string, unknown>)[name]
    if (value !== undefined && typeof value !== 'function') {
      throw new TypeError(`Actly: observability.${name} must be a function, got ${typeof value}`)
    }
  }
}
...
export function assertOptions(options: ActOptions): void {
  ...
  if (options.observability !== undefined && options.observability !== null) {
    assertObservabilityHooks(options.observability)
  }
  ...
  if (options.fallback)       assertFallbackOptions(options.fallback)
...
export function assertFallbackOptions<T>(opts: FallbackOptions<T>): void {
  if (typeof opts !== 'object' || opts === null) {
    throw new TypeError(
      `Actly: fallback must be an object with a "value" field, got ${opts === null ? 'null' : typeof opts}`,
    )
  }
  if (!('value' in opts)) {
    throw new TypeError(
      'Actly: fallback must have a "value" field (a value or a () => T | Promise<T> function). ' +
      'A fallback without "value" would resolve every failure as ok: true with value undefined.',
    )
  }
}
